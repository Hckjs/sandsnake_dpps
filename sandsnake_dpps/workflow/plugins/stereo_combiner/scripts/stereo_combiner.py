...
from ctapipe.reco.preprocessing import horizontal_to_telescope, telescope_to_horizontal
    ...
    def _combine_altaz(self, event):
        ...
        if len(fov_lon_values) > 1:
            ...
            fov_lon_weighted_average = (
                np.average(fov_lons, weights=comb_weights) * u.deg
            )
            fov_lat_weighted_average = (
                np.average(fov_lats, weights=comb_weights) * u.deg
            )
            valid = True

        elif len(fov_lon_values) == 1:
            fov_lon_weighted_average = (
                hillas_fov_lon + disp * np.cos(hillas_psi)
            ) * u.deg
            fov_lat_weighted_average = (
                hillas_fov_lat + disp * np.sin(hillas_psi)
            ) * u.deg
            valid = True

        else:
            fov_lon_weighted_average = fov_lat_weighted_average = u.Quantity(
                np.nan, u.deg
            )
            valid = False
...
class StereoKMeansCombiner(StereoCombiner):
    ...
    def _combine_altaz(self, event):
        ...
        if len(points) > 2:
            ...
            lon = centroids[best_cluster, 0] * u.deg
            lat = centroids[best_cluster, 1] * u.deg
            valid = True

        elif len(points) == 1:
            lon = (hillas_fov_lon + disp * np.cos(hillas_psi)) * u.deg
            lat = (hillas_fov_lat + disp * np.sin(hillas_psi)) * u.deg
            valid = True
...
class StereoDBScanCombiner(StereoCombiner):
    ...
    def _combine_altaz(self, event):
        ...
        if len(points) > 2:
            ...
            lon = weighted_mean[0] * u.deg
            lat = weighted_mean[1] * u.deg
            valid = True
